...
static int vfunc(int count, ...)
{
    ...
    va_copy(cap, ap);
    i1 = va_arg(ap, int);
    TEST_RESOLVED(PPC, "http://ellcc.org/bugzilla/show_bug.cgi?id=21") {
        ci1 = va_arg(cap, int);
...
static void vfunc3(int count, ...)
{
    ...
    d = va_arg(ap, double);
    TEST_RESOLVED(MIPSEL, "http://ellcc.org/bugzilla/show_bug.cgi?id=74")
    TEST_RESOLVED(MIPS, "http://ellcc.org/bugzilla/show_bug.cgi?id=74")
    TEST_RESOLVED(ARM, "http://ellcc.org/bugzilla/show_bug.cgi?id=52")
