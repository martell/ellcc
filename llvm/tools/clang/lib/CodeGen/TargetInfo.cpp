...
Address MBlazeABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  // StackAlignInBytes.
  unsigned StackAlignInBytes = 4;       // RICH: For now.
  TyInfo.second =
    std::min(TyInfo.second, CharUnits::fromQuantity(StackAlignInBytes));

  // MinABIStackAlignInBytes is the size of argument slots on the stack.
  unsigned MinABIStackAlignInBytes = 4; // RICH: For now.
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::mblaze:
    return SetCGInfo(new MBlazeTargetCodeGenInfo(Types));
