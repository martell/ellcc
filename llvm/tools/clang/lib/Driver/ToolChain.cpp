...
const DriverSuffix *parseDriverSuffix(StringRef ProgName, bool& isELLCC) {
...
ToolChain::getTargetAndModeFromProgramName(StringRef PN, bool& isELLCC) {
  std::string ProgName = normalizeProgramName(PN);
  const DriverSuffix *DS = parseDriverSuffix(ProgName, isELLCC);
  if (!DS)
  ...
  std::string Target;
  if (isELLCC || llvm::TargetRegistry::lookupTarget(Prefix, IgnoredError)) {
    Target = Prefix;
  }
